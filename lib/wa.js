...
// Public HTTPS image URL used for the IMAGE header of the confirm_order_details template.
// This should point to the same kind of image you configured in the template (static or representative banner).
...
export async function sendOrderReviewTemplate(to, urlForCheckout) {
  ...
  // with an image parameter. The template in Meta should be configured with
  // an image header that accepts a media parameter.
  if (CONFIRM_HEADER_IMAGE_URL) {
    components.push({
      type: 'header',
      parameters: [
        {
          type: 'image',
          image: { link: CONFIRM_HEADER_IMAGE_URL }
        }
      ]
    });
  }
