...
export async function sendOrderReviewTemplate(to, urlForCheckout, langCode) {
  ...
  const lc = (langCode || 'en').toString().toLowerCase();
  let templateName = 'confirm_order_details';
  let templateLang = 'en';
  if (lc === 'hi') {
    templateName = 'confirm_order_details_hi';
    templateLang = 'hi';
  } else if (lc === 'te') {
    templateName = 'confirm_order_details_te';
    templateLang = 'te';
  } else if (lc === 'ta') {
    templateName = 'confirm_order_details_ta';
    templateLang = 'ta';
  } else if (lc === 'kn' || lc === 'ka') {
    templateName = 'confirm_order_details_ka';
    templateLang = 'kn';
  } else if (lc === 'ml' || lc === 'ma') {
    templateName = 'confirm_order_details_ma';
    templateLang = 'ml';
  }
...
// Internal owner/ops alert for new wholesale orders using the
// 'internal_order_alert' template (English).
// Template structure:
//   Header (static text): "New Wholesale Order Placed!!!"
//   Body:
//     Order ID: *{{1}}*
//     Buyer: *{{2}}*
//     Whatsapp Number: *{{3}}*
//     Total: *{{4}}*
export async function sendInternalOrderAlertTemplate(to, orderId, buyerName, buyerWaNumber, totalStr) {
  const params = [
    { type: 'text', text: orderId || '-' },
    { type: 'text', text: buyerName || '-' },
    { type: 'text', text: buyerWaNumber || '-' },
