...
export async function fetchSetProductsDetailed(setId) {
  if (!setId) return [];
  let url = `https://graph.facebook.com/v20.0/${setId}/products?fields=id,retailer_id,name,description,price,currency,image_url,additional_image_urls&limit=100`;
  const out = [];
  for (let i = 0; i < 30 && url; i++) {
    const j = await graphGet(url).catch(() => ({}));
    const data = Array.isArray(j.data) ? j.data : [];
    for (const p of data) {
      // Some Commerce Manager surfaces may not include description on the set listing.
      // If it's missing or empty, best-effort fetch from the product details endpoint.
      let description = (p.description || '').toString();
      if (!description && p.id) {
        const detail = await graphGet(`https://graph.facebook.com/v20.0/${p.id}?fields=description`).catch(() => ({}));
